...
const LabelInputContainer = ({
  ...
  return (
    <div
      className={cn(
        'relative flex w-full flex-col space-y-2 overflow-hiddenrounded-xl border border-white/20 bg-slate-950 p-4',
        className
      )}
    >
      {children}
...
const QuestionForm = ({ question }: { question?: Models.Document }) => {
  ...
  const submit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      router.push(`/questions/${response.$id}/`); // TODO: Add slugify function to generate slugs
    } catch (error: unknown) {
      setError(() =>
    ...
    <form className="space-y-4" onSubmit={submit}>
      ...
      <LabelInputContainer>
        ...
        <Input
          id="title"
          name="title"
          placeholder="e.g. How to use Zustand with Next.js?"
          type="text"
      ...
      <LabelInputContainer>
        ...
        <Input
          ...
          name="image"
          accept="image/*"
          placeholder="e.g. Why is this happening?"
          type="file"
          onChange={e => {
